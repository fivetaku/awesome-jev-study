...
[한국어](../../ko/projects/361-fast-jev-compaction.md) · [English](../../en/projects/361-fast-jev-compaction.md)

- List: [kraayenjon/awesome-jev](https://github.com/kraayenjon/awesome-jev/blob/22570dcd8662ae039860a1dfad7d7aec4aff8e15/README.md#L361) `README.md:361` at `22570dcd` (CC0). The fivetaku/awesome-jev fork was cloned at this commit.
...
- Run-confirmed, GitHub API 2026-09-22: `tamaratran/fast-jev-compaction`, 6214 stars, SPDX MIT, push 2026-09-18, archived False, language TypeScript.
- The list's CC0 does not cover this repository. Its license is MIT.
- API description: Claude Code plugin that replaces the compaction summary with Jev decisions: every tool call and result is scored in one fast request, stale ones are dropped or truncated, everythin

## Role Jev plays

Jev is exposed as a tool an agent can call, or as a gate in front of an agent that already writes text. The list says: Claude Code plugin and npm library: Jev scores tool calls and drops stale ones instead of summarizing context. The agent loop, the permissions, and the side effects are not Jev. This page did not trace them.
