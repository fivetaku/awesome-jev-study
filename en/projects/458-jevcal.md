...
- List: [kraayenjon/awesome-jev](https://github.com/kraayenjon/awesome-jev/blob/22570dcd8662ae039860a1dfad7d7aec4aff8e15/README.md#L458) `README.md:458` at `22570dcd` (CC0). The fivetaku/awesome-jev fork was cloned at this commit.
...
- Run-confirmed, GitHub API 2026-09-22: `abhixhek/jevcal`, 10 stars, SPDX MIT, push 2026-09-18, archived False, language Python.
- The list's CC0 does not cover this repository. Its license is MIT.
- API description: Stop guessing confidence thresholds: calibrate, threshold, and drift-check typed decision models (TypeSafe Jev) against an LLM teacher.

## Role Jev plays

This row is a harness or a published eval, not a product feature. The list says: CLI that fits a per-question confidence threshold to a target accuracy on your own labeled data, and fails CI when a Jev update breaks locked thresholds. Official numbers on the list are vendor-reported. This study did not rerun the harness. A fetch of evals.typesafe.ai on 2026-09-22 did not contain the headline percentages an earlier, longer extract had quoted, so those percentages are not restated here.
