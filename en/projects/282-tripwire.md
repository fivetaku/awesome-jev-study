...
- List: [kraayenjon/awesome-jev](https://github.com/kraayenjon/awesome-jev/blob/22570dcd8662ae039860a1dfad7d7aec4aff8e15/README.md#L282) `README.md:282` at `22570dcd` (CC0). The fivetaku/awesome-jev fork was cloned at this commit.
...
- Run-confirmed, GitHub API 2026-09-22: `noelzappy/tripwire`, 2 stars, SPDX MIT, push 2026-09-18, archived False, language TypeScript.
- The list's CC0 does not cover this repository. Its license is MIT.
- API description: Judge every LLM response before the user sees it. AI SDK middleware and OpenAI-compatible proxy.

## Role Jev plays

The list says: AI SDK middleware and OpenAI-compatible proxy: seven Jev checks on every LLM response in ~100 ms, confidence-gated. Rows filed in this section are usually a judgment over a diff, a log, a commit, or a string already extracted, and ordinary code keeps the filesystem and the git calls. That is the section's shape. It is not a finding that this row matches it. This page did not trace the code.
