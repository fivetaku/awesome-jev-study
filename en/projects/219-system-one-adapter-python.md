...
- List: [kraayenjon/awesome-jev](https://github.com/kraayenjon/awesome-jev/blob/22570dcd8662ae039860a1dfad7d7aec4aff8e15/README.md#L219) `README.md:219` at `22570dcd` (CC0). The fivetaku/awesome-jev fork was cloned at this commit.
...
- Run-confirmed, GitHub API 2026-09-22: `typesafe-ai/system-one-adapter-python`, 254 stars, SPDX MIT, push 2026-09-22, archived False, language Python.
- The list's CC0 does not cover this repository. Its license is MIT.
- API description: Drop-in TypeSafeClient replacement backed by LLM APIs

## Role Jev plays

This repository is a client, not the model. Jev stays at the remote System One endpoint. The package is the call in one language. The list's sentence: Drop-in `TypeSafeClient` replacement backed by LLM APIs, to compare Jev against chat models on the same questions. `pip install system-one-adapter`.
...
The list sentence names: `TypeSafeClient`, `pip install system-one-adapter`. That is the list's sentence, not a config file traced in source.
