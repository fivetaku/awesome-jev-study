...
코딩 에이전트의 닫힌 판단을 Jev로 보내, 판정이 타입이 있고 싸고 실행끼리 비교되게 하는 스킬.
...
Jev는 에이전트가 부르는 도구로 열려 있거나, 이미 문장을 쓰는 에이전트 앞의 게이트다. 목록은 이렇게 적는다. 코딩 에이전트의 닫힌 판단을 Jev로 보내, 판정이 타입이 있고 싸고 실행끼리 비교되게 하는 스킬. 에이전트 루프, 권한, 그 다음 일은 Jev가 아니다. 이 페이지는 그것들을 따라가지 않았다.
