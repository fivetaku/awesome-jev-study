...
이 행은 제품 기능이 아니라 하네스이거나 공개 평가다. 목록은 이렇게 적는다. CLI that fits a per-question confidence threshold to a target accuracy on your own labeled data, and fails CI when a Jev update breaks locked thresholds. 목록의 공식 숫자는 벤더 보고다. 이 스터디는 하네스를 다시 돌리지 않았다. 2026-09-22의 evals.typesafe.ai 조회 본문에는, 더 긴 이전 추출이 인용하던 대표 백분율이 없었다. 그래서 그 백분율은 여기 다시 적지 않는다.
