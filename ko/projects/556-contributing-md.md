...
- 목록: [kraayenjon/awesome-jev](https://github.com/kraayenjon/awesome-jev/blob/22570dcd8662ae039860a1dfad7d7aec4aff8e15/README.md#L556) `README.md:556` @ `22570dcd` (CC0). fivetaku/awesome-jev 포크는 이 커밋으로 클론했다.
- 절: 라이선스
- 주소: local:CONTRIBUTING.md
- 조회: 2026-09-22
...
[CONTRIBUTING.md](CONTRIBUTING.md)를 본다. 짧게는, 프로젝트에 링크와 한 줄 설명을 더하는 풀 리퀘스트를 연다. 쓸모 있고, 흥미롭고, 실제로 Jev 위에 지었거나 그 인터페이스에서 분명히 영감을 받아야 한다. 실험이거나 드라이런만 하는 경로(매매, 홈 자동화)는 명시한다.
...
이 행은 Jev 통합이 아니다. Jev에 대한 페이지, 글, 목록, FAQ 답이다. 목록은 이렇게 적는다. [CONTRIBUTING.md](CONTRIBUTING.md)를 본다. 짧게는, 프로젝트에 링크와 한 줄 설명을 더하는 풀 리퀘스트를 연다. 쓸모 있고, 흥미롭고, 실제로 Jev 위에 지었거나 그 인터페이스에서 분명히 영감을 받아야 한다. 실험이거나 드라이런만 하는 경로(매매, 홈 자동화)는 명시한다. 프로젝트가 아닌 앞부분에서 복구할 호출 설정은 없다. 소셜 글의 문장은 확인된 사실로 취급하지 않는다.
