...
- 목록: [kraayenjon/awesome-jev](https://github.com/kraayenjon/awesome-jev/blob/22570dcd8662ae039860a1dfad7d7aec4aff8e15/README.md#L374) `README.md:374` @ `22570dcd` (CC0). fivetaku/awesome-jev 포크는 이 커밋으로 클론했다.
- 절: 에이전트 도구·MCP
- 주소: https://github.com/leepokai/jev-guard
- 조회: 2026-09-22
...
Claude Code, Codex, Pi, ACP 에이전트용 프롬프트 주입과 위험한 행동 가드.

## 확인한 것

- 실행으로 확인, GitHub API 2026-09-22: `leepokai/jev-guard`, 스타 22, SPDX MIT, push 2026-09-17, 아카이브 False, 언어 JavaScript.
- 목록의 CC0은 이 저장소 코드에 미치지 않는다. 라이선스는 MIT이다.
- API 설명 한 줄: Auto mode for every coding agent, built on Jev: risk-scores every tool call with session context (deny / ask / allow), flags prompt injection in results, checks skills and plugins.

## Jev의 역할

Jev는 에이전트가 부르는 도구로 열려 있거나, 이미 문장을 쓰는 에이전트 앞의 게이트다. 목록은 이렇게 적는다. Claude Code, Codex, Pi, ACP 에이전트용 프롬프트 주입과 위험한 행동 가드. 에이전트 루프, 권한, 그 다음 일은 Jev가 아니다. 이 페이지는 그것들을 따라가지 않았다.
