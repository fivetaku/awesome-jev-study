...
- 목록: [kraayenjon/awesome-jev](https://github.com/kraayenjon/awesome-jev/blob/22570dcd8662ae039860a1dfad7d7aec4aff8e15/README.md#L296) `README.md:296` @ `22570dcd` (CC0). fivetaku/awesome-jev 포크는 이 커밋으로 클론했다.
- 절: 모델 라우팅
- 주소: https://github.com/mejiasd3v/pi-jev-router
- 조회: 2026-09-22
...
Pi 코딩 에이전트의 요청마다 모델을 고른다. 결정은 Vercel AI Gateway 위의 Jev.

## 확인한 것

- 실행으로 확인, GitHub API 2026-09-22: `mejiasd3v/pi-jev-router`, 스타 12, SPDX MIT, push 2026-09-22, 아카이브 False, 언어 JavaScript.
- 목록의 CC0은 이 저장소 코드에 미치지 않는다. 라이선스는 MIT이다.
- API 설명 한 줄: Automatic model routing for Pi using TypeSafe's Jev through Vercel AI Gateway

## Jev의 역할

Jev의 Choice가 다음 모델, 스킬, 경로를 고른다. 생성이 있다면 그 선택 다음에 일어난다. 목록은 이렇게 적는다. Pi 코딩 에이전트의 요청마다 모델을 고른다. 결정은 Vercel AI Gateway 위의 Jev. 이 페이지는 라우팅 규칙을 소스에서 따라가지 않았다.
