...
- 목록: [kraayenjon/awesome-jev](https://github.com/kraayenjon/awesome-jev/blob/22570dcd8662ae039860a1dfad7d7aec4aff8e15/README.md#L432) `README.md:432` @ `22570dcd` (CC0). fivetaku/awesome-jev 포크는 이 커밋으로 클론했다.
- 절: 따라 하는 예제
- 주소: https://docs.typesafe.ai/cookbooks/classifying_rag_passages
- 조회: 2026-09-22
...
답하는 모델 전에, 가져온 구절을 남기거나, 표시하거나, 버린다.
...
이 행은 레시피다. 목록은 이렇게 적는다. 답하는 모델 전에, 가져온 구절을 남기거나, 표시하거나, 버린다. 목록이 보여주는 설정은 `state`와 Choice·Noul·Score 질문이고, 임계값은 코드에 남긴다. 레시피는 실행하지 않았다. 커뮤니티 쿡북 코드는 복사하지 않았다.
