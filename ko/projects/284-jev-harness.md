...
Jev 주위의 운영 층. 정책, 확신 게이트, 그림자 모드, 레시피, 평가 CLI.
...
목록은 이렇게 적는다. Jev 주위의 운영 층. 정책, 확신 게이트, 그림자 모드, 레시피, 평가 CLI. 이 절에 모인 행은 대개 diff, 로그, 커밋, 이미 뽑아 둔 문자열에 대한 판단이고, 파일시스템과 git 호출은 일반 코드가 가진다. 그것은 절의 모양이다. 이 행이 그렇게 생겼다는 확인은 아니다. 이 페이지는 그 코드를 따라가지 않았다.
