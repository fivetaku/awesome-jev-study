...
- 목록: [kraayenjon/awesome-jev](https://github.com/kraayenjon/awesome-jev/blob/22570dcd8662ae039860a1dfad7d7aec4aff8e15/README.md#L248) `README.md:248` @ `22570dcd` (CC0). fivetaku/awesome-jev 포크는 이 커밋으로 클론했다.
- 절: 브라우저·컴퓨터 조작
- 주소: https://github.com/droidrun/mobile-jev
- 조회: 2026-09-22
...
Mobilerun 위의 Android 에이전트. Jev가 탭을 정한다. Uber를 열어 SFO에서 골든게이트까지, 결제 화면까지 약 21초, 행동 9번. ADB는 없다.

## 확인한 것

- 실행으로 확인, GitHub API 2026-09-22: `droidrun/mobile-jev`, 스타 341, SPDX MIT, push 2026-09-17, 아카이브 False, 언어 JavaScript.
- 목록의 CC0은 이 저장소 코드에 미치지 않는다. 라이선스는 MIT이다.

## Jev의 역할

목록이 맞다면 Jev의 자리는 브라우저나 컴퓨터 조작 루프 안의 결정이다. 목록은 이렇게 적는다. Mobilerun 위의 Android 에이전트. Jev가 탭을 정한다. Uber를 열어 SFO에서 골든게이트까지, 결제 화면까지 약 21초, 행동 9번. ADB는 없다. 페이지를 읽고 클릭을 실행하는 일은 일반 코드다. 이 페이지는 그 루프를 따라가지 않았고, 이 프로젝트가 픽셀을 본다고 말하지 않는다. 텍스트 상태이지 프레임이 아니라고 적은 것은 Doom 소개뿐이며, 그 문장을 다른 에이전트에 옮기지 않는다.
