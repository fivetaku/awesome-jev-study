...
자연어 요청을, 인자가 닫힌 집합인 보통의 타입이 있는 함수에 대응시킨다.
...
이 행은 레시피다. 목록은 이렇게 적는다. 자연어 요청을, 인자가 닫힌 집합인 보통의 타입이 있는 함수에 대응시킨다. 목록이 보여주는 설정은 `state`와 Choice·Noul·Score 질문이고, 임계값은 코드에 남긴다. 레시피는 실행하지 않았다. 커뮤니티 쿡북 코드는 복사하지 않았다.
