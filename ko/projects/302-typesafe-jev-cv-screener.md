...
목록은 이렇게 적는다. Screens a folder of CVs against an editable policy; re-scoring candidates is free when the policy changes. 이 절의 행은 대개 닫힌 라벨이다. 채용 여부, 의도, 매수·매도, 허용·차단. 돈, 메시지, 기록은 일반 코드에 남는다. 그 예는 절의 예이고, 이 행이 그중 하나라는 확인은 아니다. 이 페이지는 그 부수 효과를 따라가지 않았다.
