...
[한국어](../../ko/projects/302-typesafe-jev-cv-screener.md) · [English](../../en/projects/302-typesafe-jev-cv-screener.md)

- 목록: [kraayenjon/awesome-jev](https://github.com/kraayenjon/awesome-jev/blob/22570dcd8662ae039860a1dfad7d7aec4aff8e15/README.md#L302) `README.md:302` @ `22570dcd` (CC0). fivetaku/awesome-jev 포크는 이 커밋으로 클론했다.
- 절: 업무용 앱
- 주소: https://github.com/gtaras7/typesafe-jev
- 조회: 2026-09-22
...
이력서 폴더를 고칠 수 있는 정책에 비춘다. 정책이 바뀌면 후보를 다시 채점하는 비용은 없다고 목록이 적는다.

## 확인한 것

- 실행으로 확인, GitHub API 2026-09-22: `gtaras7/typesafe-jev`, 스타 4, SPDX MIT, push 2026-09-17, 아카이브 False, 언어 TypeScript.
- 목록의 CC0은 이 저장소 코드에 미치지 않는다. 라이선스는 MIT이다.
- API 설명 한 줄: Screen a folder of CVs with the TypeSafe Jev decision model: typed judgments, an editable policy, free re-scoring.

## Jev의 역할

목록은 이렇게 적는다. 이력서 폴더를 고칠 수 있는 정책에 비춘다. 정책이 바뀌면 후보를 다시 채점하는 비용은 없다고 목록이 적는다. 이 절의 행은 대개 닫힌 라벨이다. 채용 여부, 의도, 매수·매도, 허용·차단. 돈, 메시지, 기록은 일반 코드에 남는다. 그 예는 절의 예이고, 이 행이 그중 하나라는 확인은 아니다. 이 페이지는 그 다음 일을 따라가지 않았다.
