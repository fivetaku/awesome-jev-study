...
- 목록: [kraayenjon/awesome-jev](https://github.com/kraayenjon/awesome-jev/blob/22570dcd8662ae039860a1dfad7d7aec4aff8e15/README.md#L196) `README.md:196` @ `22570dcd` (CC0). fivetaku/awesome-jev 포크는 이 커밋으로 클론했다.
- 절: 숫자가 붙은 빌드
- 주소: https://madewithjev.com/builds/stagehand-remote-browser
- 조회: 2026-09-22
...
브라우저 작업 한 건이 1센트의 10분의 1이라고 목록이 적는다.

## 확인한 것

- 목록이 같이 적은 숫자: ~$0.001/task. 저자 보고이다 (README.md:209). 재측정하지 않았다.

## Jev의 역할

목록이 맞다면 Jev의 자리는 브라우저나 컴퓨터 조작 루프 안의 결정이다. 목록은 이렇게 적는다. 브라우저 작업 한 건이 1센트의 10분의 1이라고 목록이 적는다. 페이지를 읽고 클릭을 실행하는 일은 일반 코드다. 이 페이지는 그 루프를 따라가지 않았고, 이 프로젝트가 픽셀을 본다고 말하지 않는다. 텍스트 상태이지 프레임이 아니라고 적은 것은 Doom 소개뿐이며, 그 문장을 다른 에이전트에 옮기지 않는다.
