...
Jev는 에이전트가 부르는 도구로 열려 있거나, 이미 문장을 쓰는 에이전트 앞의 게이트다. 목록은 이렇게 적는다. Rust CLI: Jev ranks which agent skill fits the next step from live session context, with Claude Code hooks. 에이전트 루프, 권한, 부수 효과는 Jev가 아니다. 이 페이지는 그것들을 따라가지 않았다.
