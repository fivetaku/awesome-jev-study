...
System One 판단을 Pi 도구 다섯 개로 연다. 임계값, 가중치, 행동은 코드와 사용자가 쥔다.
...
Jev는 에이전트가 부르는 도구로 열려 있거나, 이미 문장을 쓰는 에이전트 앞의 게이트다. 목록은 이렇게 적는다. System One 판단을 Pi 도구 다섯 개로 연다. 임계값, 가중치, 행동은 코드와 사용자가 쥔다. 에이전트 루프, 권한, 그 다음 일은 Jev가 아니다. 이 페이지는 그것들을 따라가지 않았다.
