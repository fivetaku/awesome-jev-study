...
- 목록: [kraayenjon/awesome-jev](https://github.com/kraayenjon/awesome-jev/blob/22570dcd8662ae039860a1dfad7d7aec4aff8e15/README.md#L376) `README.md:376` @ `22570dcd` (CC0). fivetaku/awesome-jev 포크는 이 커밋으로 클론했다.
- 절: 에이전트 도구·MCP
- 주소: https://github.com/valentynkit/jev-belay
- 조회: 2026-09-22
...
Claude Code Stop 훅. 전사를 읽어 일이 끝났다는 증거가 있는지 본다. 파일이 바뀌었는데 그 뒤 통과한 검사가 없을 때만 질문 네 개의 Jev 호출을 한 번 쓰고 멈추게 한다. 오류 경로는 모두 열어 둔 채 실패한다.

## 확인한 것

- 실행으로 확인, GitHub API 2026-09-22: `valentynkit/jev-belay`, 스타 17, SPDX MIT, push 2026-09-20, 아카이브 False, 언어 JavaScript.
- 목록의 CC0은 이 저장소 코드에 미치지 않는다. 라이선스는 MIT이다.
- API 설명 한 줄: Claude Code Stop hook that blocks an unverified done: reads the transcript for evidence, asks Jev once, fails open on everything else

## Jev의 역할

Jev는 에이전트가 부르는 도구로 열려 있거나, 이미 문장을 쓰는 에이전트 앞의 게이트다. 목록은 이렇게 적는다. Claude Code Stop 훅. 전사를 읽어 일이 끝났다는 증거가 있는지 본다. 파일이 바뀌었는데 그 뒤 통과한 검사가 없을 때만 질문 네 개의 Jev 호출을 한 번 쓰고 멈추게 한다. 오류 경로는 모두 열어 둔 채 실패한다. 에이전트 루프, 권한, 그 다음 일은 Jev가 아니다. 이 페이지는 그것들을 따라가지 않았다.
