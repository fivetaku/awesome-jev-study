...
오리지널 스타크래프트 셰어웨어 캠페인을 구조화된 상태로 두는 하네스. 확인된 실행과 확률 추적이 있다.
...
Jev의 자리는 다음 합법 수다. 게임 코드가 상태나 선택지로 만들어 넘긴 것 중에서 고른다. 목록은 이렇게 적는다. 오리지널 스타크래프트 셰어웨어 캠페인을 구조화된 상태로 두는 하네스. 확인된 실행과 확률 추적이 있다. 게임, 합법 수 생성, 입력 주입은 모델 밖에 있다. 이 페이지는 그것들을 따라가지 않았고, 저자가 보고한 속도·비용을 재측정하지 않았다.
