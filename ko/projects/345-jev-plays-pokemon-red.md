...
PyBoy 위의 포켓몬 레드. 경로와 계산은 코드가 갖고, Jev는 갈림에서만 고른다. 전투 턴마다 기절 예측을 남기고, RAM이 말한 결과와 Brier로 점수를 매긴다.
...
Jev의 자리는 다음 합법 수다. 게임 코드가 상태나 선택지로 만들어 넘긴 것 중에서 고른다. 목록은 이렇게 적는다. PyBoy 위의 포켓몬 레드. 경로와 계산은 코드가 갖고, Jev는 갈림에서만 고른다. 전투 턴마다 기절 예측을 남기고, RAM이 말한 결과와 Brier로 점수를 매긴다. 게임, 합법 수 생성, 입력 주입은 모델 밖에 있다. 이 페이지는 그것들을 따라가지 않았고, 저자가 보고한 속도·비용을 재측정하지 않았다.
