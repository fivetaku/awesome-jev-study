...
[한국어](../../ko/projects/306-human-compiler.md) · [English](../../en/projects/306-human-compiler.md)

- 목록: [kraayenjon/awesome-jev](https://github.com/kraayenjon/awesome-jev/blob/22570dcd8662ae039860a1dfad7d7aec4aff8e15/README.md#L306) `README.md:306` @ `22570dcd` (CC0). fivetaku/awesome-jev 포크는 이 커밋으로 클론했다.
- 절: 업무용 앱
- 주소: https://github.com/asfarsadewa/human-compiler
- 조회: 2026-09-22
...
회사 문장을 붙이면 Jev가 수동공격, 긴급, 정보 밀도를 점수로 매기고, 코드가 rustc 스타일 진단을 낸다. 라이브는 [human-compiler.asfarlab.fun](https://human-compiler.asfarlab.fun).

## 확인한 것

- 실행으로 확인, GitHub API 2026-09-22: `asfarsadewa/human-compiler`, 스타 0, SPDX MIT, push 2026-09-17, 아카이브 False, 언어 TypeScript.
- 목록의 CC0은 이 저장소 코드에 미치지 않는다. 라이선스는 MIT이다.
- API 설명 한 줄: A compiler for human language. Paste text, get diagnostics. Measured by TypeSafe Jev.

## Jev의 역할

목록은 이렇게 적는다. 회사 문장을 붙이면 Jev가 수동공격, 긴급, 정보 밀도를 점수로 매기고, 코드가 rustc 스타일 진단을 낸다. 라이브는 [human-compiler.asfarlab.fun](https://human-compiler.asfarlab.fun). 이 절의 행은 대개 닫힌 라벨이다. 채용 여부, 의도, 매수·매도, 허용·차단. 돈, 메시지, 기록은 일반 코드에 남는다. 그 예는 절의 예이고, 이 행이 그중 하나라는 확인은 아니다. 이 페이지는 그 다음 일을 따라가지 않았다.
