...
Jev의 Choice가 다음 모델, 스킬, 경로를 고른다. 생성이 있다면 그 선택 다음에 일어난다. 목록은 이렇게 적는다. Per-turn routing for Claude Code and Codex: simple work to the fast tier, hard work to the strong tier. `npm i -g jev-router`. 이 페이지는 라우팅 규칙을 소스에서 따라가지 않았다.
