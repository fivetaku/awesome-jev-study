...
[한국어](../../ko/projects/280-typeful-triage.md) · [English](../../en/projects/280-typeful-triage.md)

- 목록: [kraayenjon/awesome-jev](https://github.com/kraayenjon/awesome-jev/blob/22570dcd8662ae039860a1dfad7d7aec4aff8e15/README.md#L280) `README.md:280` @ `22570dcd` (CC0). fivetaku/awesome-jev 포크는 이 커밋으로 클론했다.
- 절: 개발 도구·코드 리뷰
- 주소: https://github.com/cephalization/jev-triage
- 조회: 2026-09-22
...
여러 사람이 쓰는 이슈 분류 대시보드. 이슈마다 종류, 심각도, 긴급, 중복, 다음 단계가 고정된 질문이다. 사람이 고친 답은 다음 실행에서 모델에 다시 보여 준다.

## 확인한 것

- 실행으로 확인, GitHub API 2026-09-22: `cephalization/jev-triage`, 스타 1, SPDX MIT, push 2026-09-20, 아카이브 False, 언어 TypeScript.
- 목록의 CC0은 이 저장소 코드에 미치지 않는다. 라이선스는 MIT이다.
- API 설명 한 줄: Uses typeful jev, zero sync to pull and sync large repositories for issue triage

## Jev의 역할

목록은 이렇게 적는다. 여러 사람이 쓰는 이슈 분류 대시보드. 이슈마다 종류, 심각도, 긴급, 중복, 다음 단계가 고정된 질문이다. 사람이 고친 답은 다음 실행에서 모델에 다시 보여 준다. 이 절에 모인 행은 대개 diff, 로그, 커밋, 이미 뽑아 둔 문자열에 대한 판단이고, 파일시스템과 git 호출은 일반 코드가 가진다. 그것은 절의 모양이다. 이 행이 그렇게 생겼다는 확인은 아니다. 이 페이지는 그 코드를 따라가지 않았다.
