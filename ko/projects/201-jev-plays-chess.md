...
둘 수 있는 수를 Choice로 두고, 추론 모델과 비교한다.
...
Jev의 자리는 다음 합법 수다. 게임 코드가 상태나 선택지로 만들어 넘긴 것 중에서 고른다. 목록은 이렇게 적는다. 둘 수 있는 수를 Choice로 두고, 추론 모델과 비교한다. 게임, 합법 수 생성, 입력 주입은 모델 밖에 있다. 이 페이지는 그것들을 따라가지 않았고, 저자가 보고한 속도·비용을 재측정하지 않았다.
