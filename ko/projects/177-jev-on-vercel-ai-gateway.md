...
[한국어](../../ko/projects/177-jev-on-vercel-ai-gateway.md) · [English](../../en/projects/177-jev-on-vercel-ai-gateway.md)

- 목록: [kraayenjon/awesome-jev](https://github.com/kraayenjon/awesome-jev/blob/22570dcd8662ae039860a1dfad7d7aec4aff8e15/README.md#L177) `README.md:177` @ `22570dcd` (CC0). fivetaku/awesome-jev 포크는 이 커밋으로 클론했다.
- 절: 공식 자료
- 주소: https://vercel.com/ai-gateway/models/jev
- 조회: 2026-09-22
...
AI SDK의 `experimental_evaluate`용으로 호스팅된 `typesafe-ai/jev`. 목록은 TypeSafe 대기 명단이 필요 없다고 적는다. 이 페이지의 가격은 여기서 확인하지 않았다.

## 확인한 것

- 이번 Vercel 조회에는 모델 id `typesafe-ai/jev`가 있고, 가격 화면 표시에는 32K와 Free가 있다. 본문에 `0.042`도 `waitlist`도 없다. 목록의 가격과 '대기 명단 없음'을 이 페이지가 확인했다고 말하지 않는다.

## Jev의 역할

이 행은 커뮤니티 통합이 아니라 TypeSafe 페이지이거나 목록 자신의 표다. 다루는 것: AI SDK의 `experimental_evaluate`용으로 호스팅된 `typesafe-ai/jev`. 목록은 TypeSafe 대기 명단이 필요 없다고 적는다. 이 페이지의 가격은 여기서 확인하지 않았다.

## 설정

설정으로 적는 것은 목록 문장과 2026-09-22에 읽은 앞부분뿐이다. 저장소 소스는 열지 않았다.

목록 문장이 이름까지 적는다: `typesafe-ai/jev`. 그것은 목록의 문장이지, 소스를 따라가 확인한 설정 파일이 아니다.
