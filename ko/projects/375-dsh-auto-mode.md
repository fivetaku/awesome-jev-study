...
- 목록: [kraayenjon/awesome-jev](https://github.com/kraayenjon/awesome-jev/blob/22570dcd8662ae039860a1dfad7d7aec4aff8e15/README.md#L375) `README.md:375` @ `22570dcd` (CC0). fivetaku/awesome-jev 포크는 이 커밋으로 클론했다.
- 절: 에이전트 도구·MCP
- 주소: https://git.allen-software.com/allenh1/dsh-auto-mode
- 조회: 2026-09-22
...
DeepSeek Harness 권한 프리셋. 끝 프롬프트 단계에서, 에이전트가 마지막 메시지에 남긴 열린 질문을 Jev가 답한다.
...
Jev는 에이전트가 부르는 도구로 열려 있거나, 이미 문장을 쓰는 에이전트 앞의 게이트다. 목록은 이렇게 적는다. DeepSeek Harness 권한 프리셋. 끝 프롬프트 단계에서, 에이전트가 마지막 메시지에 남긴 열린 질문을 Jev가 답한다. 에이전트 루프, 권한, 그 다음 일은 Jev가 아니다. 이 페이지는 그것들을 따라가지 않았다.
