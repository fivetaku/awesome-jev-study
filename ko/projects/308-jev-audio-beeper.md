...
[한국어](../../ko/projects/308-jev-audio-beeper.md) · [English](../../en/projects/308-jev-audio-beeper.md)

- 목록: [kraayenjon/awesome-jev](https://github.com/kraayenjon/awesome-jev/blob/22570dcd8662ae039860a1dfad7d7aec4aff8e15/README.md#L308) `README.md:308` @ `22570dcd` (CC0). fivetaku/awesome-jev 포크는 이 커밋으로 클론했다.
- 절: 업무용 앱
- 주소: https://github.com/santos-sanz/jev-audio-beeper
- 조회: 2026-09-22
...
지연이 짧은 모욕 탐지기. Jev가 정하고, ffmpeg가 약 466ms 안에 삑 소리를 넣는다. 트랙의 나머지는 다시 쓰지 않는다.

## 확인한 것

- 실행으로 확인, GitHub API 2026-09-22: `santos-sanz/jev-audio-beeper`, 스타 2, SPDX none, push 2026-09-17, 아카이브 False, 언어 TypeScript.
- API 설명 한 줄: Low-latency audio censorship POC using Jev typed decisions and ffmpeg.

## Jev의 역할

목록은 이렇게 적는다. 지연이 짧은 모욕 탐지기. Jev가 정하고, ffmpeg가 약 466ms 안에 삑 소리를 넣는다. 트랙의 나머지는 다시 쓰지 않는다. 이 절의 행은 대개 닫힌 라벨이다. 채용 여부, 의도, 매수·매도, 허용·차단. 돈, 메시지, 기록은 일반 코드에 남는다. 그 예는 절의 예이고, 이 행이 그중 하나라는 확인은 아니다. 이 페이지는 그 다음 일을 따라가지 않았다.
