...
왕좌의 게임 롤플레이. 이야기 모델이 장면을 쓰고, Jev는 존 스노가 어디 있는지, 위험이 얼마인지, 그 아래에 무엇이 깔려야 하는지를 답한다.
...
Jev의 자리는 다음 합법 수다. 게임 코드가 상태나 선택지로 만들어 넘긴 것 중에서 고른다. 목록은 이렇게 적는다. 왕좌의 게임 롤플레이. 이야기 모델이 장면을 쓰고, Jev는 존 스노가 어디 있는지, 위험이 얼마인지, 그 아래에 무엇이 깔려야 하는지를 답한다. 게임, 합법 수 생성, 입력 주입은 모델 밖에 있다. 이 페이지는 그것들을 따라가지 않았고, 저자가 보고한 속도·비용을 재측정하지 않았다.
