...
요청을 넘기기 전에 Jev가 어느 모델이 받을지 정한다.
...
이 행은 Jev 통합이 아니다. Jev에 대한 페이지, 글, 목록, FAQ 답이다. 목록은 이렇게 적는다. 요청을 넘기기 전에 Jev가 어느 모델이 받을지 정한다. 프로젝트가 아닌 앞부분에서 복구할 호출 설정은 없다. 소셜 글의 문장은 확인된 사실로 취급하지 않는다.
