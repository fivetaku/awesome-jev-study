...
- 목록: [kraayenjon/awesome-jev](https://github.com/kraayenjon/awesome-jev/blob/22570dcd8662ae039860a1dfad7d7aec4aff8e15/README.md#L543) `README.md:543` @ `22570dcd` (CC0). fivetaku/awesome-jev 포크는 이 커밋으로 클론했다.
- 절: 자주 묻는 질문
- 주소: 이 행에는 URL이 없다
- 조회: 2026-09-22
...
소프트웨어 안에서 빠르고 구조화된 결정을 위해 만든 모델 종류를 TypeSafe가 부르는 이름. 사람에게 글을 생성하는 채팅 모델과 대비된다. 빠르고 직관적인 System 1 사고에서 이름을 땄다. Jev가 첫 공개 모델이다.
...
이 행은 Jev 통합이 아니다. Jev에 대한 페이지, 글, 목록, FAQ 답이다. 목록은 이렇게 적는다. 소프트웨어 안에서 빠르고 구조화된 결정을 위해 만든 모델 종류를 TypeSafe가 부르는 이름. 사람에게 글을 생성하는 채팅 모델과 대비된다. 빠르고 직관적인 System 1 사고에서 이름을 땄다. Jev가 첫 공개 모델이다. 프로젝트가 아닌 앞부분에서 복구할 호출 설정은 없다. 소셜 글의 문장은 확인된 사실로 취급하지 않는다.
