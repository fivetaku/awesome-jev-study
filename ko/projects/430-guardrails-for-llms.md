...
이 행은 레시피다. 목록은 이렇게 적는다. Screen messages in and out of an LLM; threshold probabilities in code. 목록이 보여주는 세팅은 state와 Choice·Noul·Score 질문이고, 임계값은 코드에 남긴다. 레시피는 실행하지 않았다. 커뮤니티 쿡북 코드는 복사하지 않았다.
