...
- 목록: [kraayenjon/awesome-jev](https://github.com/kraayenjon/awesome-jev/blob/22570dcd8662ae039860a1dfad7d7aec4aff8e15/README.md#L430) `README.md:430` @ `22570dcd` (CC0). fivetaku/awesome-jev 포크는 이 커밋으로 클론했다.
- 절: 따라 하는 예제
- 주소: https://docs.typesafe.ai/cookbooks/llm_guardrails
- 조회: 2026-09-22
...
LLM에 들어가고 나오는 메시지를 거른다. 확률 문턱은 코드에 둔다.
...
이 행은 레시피다. 목록은 이렇게 적는다. LLM에 들어가고 나오는 메시지를 거른다. 확률 문턱은 코드에 둔다. 목록이 보여주는 설정은 `state`와 Choice·Noul·Score 질문이고, 임계값은 코드에 남긴다. 레시피는 실행하지 않았다. 커뮤니티 쿡북 코드는 복사하지 않았다.
