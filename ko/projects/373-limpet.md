...
- 목록: [kraayenjon/awesome-jev](https://github.com/kraayenjon/awesome-jev/blob/22570dcd8662ae039860a1dfad7d7aec4aff8e15/README.md#L373) `README.md:373` @ `22570dcd` (CC0). fivetaku/awesome-jev 포크는 이 커밋으로 클론했다.
- 절: 에이전트 도구·MCP
- 주소: https://github.com/noplan-inc/limpet
- 조회: 2026-09-22
...
Stop 훅. 평범한 말로 적은 완료 규칙을 Jev가 판단해, 에이전트가 너무 일찍 끝내지 못하게 한다.

## 확인한 것

- 실행으로 확인, GitHub API 2026-09-22: `noplan-inc/limpet`, 스타 2, SPDX MIT, push 2026-09-17, 아카이브 False, 언어 Python.
- 목록의 CC0은 이 저장소 코드에 미치지 않는다. 라이선스는 MIT이다.
- API 설명 한 줄: A Stop hook that stops your coding agent from stopping too early. Plain-language rules, judged by jev.

## Jev의 역할

Jev는 에이전트가 부르는 도구로 열려 있거나, 이미 문장을 쓰는 에이전트 앞의 게이트다. 목록은 이렇게 적는다. Stop 훅. 평범한 말로 적은 완료 규칙을 Jev가 판단해, 에이전트가 너무 일찍 끝내지 못하게 한다. 에이전트 루프, 권한, 그 다음 일은 Jev가 아니다. 이 페이지는 그것들을 따라가지 않았다.
