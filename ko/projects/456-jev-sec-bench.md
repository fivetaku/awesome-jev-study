...
- 목록: [kraayenjon/awesome-jev](https://github.com/kraayenjon/awesome-jev/blob/22570dcd8662ae039860a1dfad7d7aec4aff8e15/README.md#L456) `README.md:456` @ `22570dcd` (CC0). fivetaku/awesome-jev 포크는 이 커밋으로 클론했다.
- 절: Benchmarks and evaluations (벤치마크)
- 주소: https://github.com/Gaurav-Gosain/jev-sec-bench
- 조회: 2026-09-22
...
- 실행으로 확인, GitHub API 2026-09-22: `Gaurav-Gosain/jev-sec-bench`, 스타 2, SPDX MIT, push 2026-09-16, 아카이브 False, 언어 Go.
- 목록의 CC0은 이 저장소 코드에 미치지 않는다. 라이선스는 MIT이다.
- API 설명 한 줄: Blind security benchmarks for Jev, TypeSafe's System One model: prompt injection and vulnerable code detection, built on jev-go

## Jev의 역할

이 행은 제품 기능이 아니라 하네스이거나 공개 평가다. 목록은 이렇게 적는다. Blind prompt-injection and vulnerable-code detection benches on public corpora. 목록의 공식 숫자는 벤더 보고다. 이 스터디는 하네스를 다시 돌리지 않았다. 2026-09-22의 evals.typesafe.ai 조회 본문에는, 더 긴 이전 추출이 인용하던 대표 백분율이 없었다. 그래서 그 백분율은 여기 다시 적지 않는다.
