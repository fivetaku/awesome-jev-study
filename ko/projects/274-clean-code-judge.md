...
[한국어](../../ko/projects/274-clean-code-judge.md) · [English](../../en/projects/274-clean-code-judge.md)

- 목록: [kraayenjon/awesome-jev](https://github.com/kraayenjon/awesome-jev/blob/22570dcd8662ae039860a1dfad7d7aec4aff8e15/README.md#L274) `README.md:274` @ `22570dcd` (CC0). fivetaku/awesome-jev 포크는 이 커밋으로 클론했다.
- 절: 개발 도구·코드 리뷰
- 주소: https://github.com/frostney/clean-code-review
- 조회: 2026-09-22
...
PR 파일마다 Clean Code 냄새 31개를 예/아니오로 보고, 함수 크기와 중첩도 본다. 그 판정을 글을 쓰는 모델에 넘겨 산문으로 만든다.

## 확인한 것

- 실행으로 확인, GitHub API 2026-09-22: `frostney/clean-code-review`, 스타 10, SPDX MIT, push 2026-09-21, 아카이브 False, 언어 TypeScript.
- 목록의 CC0은 이 저장소 코드에 미치지 않는다. 라이선스는 MIT이다.
- API 설명 한 줄: Every code file in a pull request, judged against Uncle Bob's Clean Code by TypeSafe's Jev, then reviewed by Luna. Built on eve and Next.js.

## Jev의 역할

목록은 이렇게 적는다. PR 파일마다 Clean Code 냄새 31개를 예/아니오로 보고, 함수 크기와 중첩도 본다. 그 판정을 글을 쓰는 모델에 넘겨 산문으로 만든다. 이 절에 모인 행은 대개 diff, 로그, 커밋, 이미 뽑아 둔 문자열에 대한 판단이고, 파일시스템과 git 호출은 일반 코드가 가진다. 그것은 절의 모양이다. 이 행이 그렇게 생겼다는 확인은 아니다. 이 페이지는 그 코드를 따라가지 않았다.
