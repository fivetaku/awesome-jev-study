...
설계 판단 스킬. Choice·Score·Noul을 고전적 방법에 대응시키고, 합성 대수, 질문 설계 진단, 반증용 검증 게이트를 둔다.
...
Jev는 에이전트가 부르는 도구로 열려 있거나, 이미 문장을 쓰는 에이전트 앞의 게이트다. 목록은 이렇게 적는다. 설계 판단 스킬. Choice·Score·Noul을 고전적 방법에 대응시키고, 합성 대수, 질문 설계 진단, 반증용 검증 게이트를 둔다. 에이전트 루프, 권한, 그 다음 일은 Jev가 아니다. 이 페이지는 그것들을 따라가지 않았다.
