...
Hermes 스킬. 에이전트에게 범위가 닫힌 결정이 필요할 때 Jev에게 묻는다.
...
Jev는 에이전트가 부르는 도구로 열려 있거나, 이미 문장을 쓰는 에이전트 앞의 게이트다. 목록은 이렇게 적는다. Hermes 스킬. 에이전트에게 범위가 닫힌 결정이 필요할 때 Jev에게 묻는다. 에이전트 루프, 권한, 그 다음 일은 Jev가 아니다. 이 페이지는 그것들을 따라가지 않았다.
