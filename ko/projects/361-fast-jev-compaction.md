...
- 목록: [kraayenjon/awesome-jev](https://github.com/kraayenjon/awesome-jev/blob/22570dcd8662ae039860a1dfad7d7aec4aff8e15/README.md#L361) `README.md:361` @ `22570dcd` (CC0). fivetaku/awesome-jev 포크는 이 커밋으로 클론했다.
- 절: 에이전트 도구·MCP
- 주소: https://github.com/tamaratran/fast-jev-compaction
- 조회: 2026-09-22
...
Claude Code 플러그인과 npm 라이브러리. 문맥을 요약하는 대신 Jev가 도구 호출에 점수를 매겨 낡은 것을 버린다.

## 확인한 것

- 실행으로 확인, GitHub API 2026-09-22: `tamaratran/fast-jev-compaction`, 스타 6214, SPDX MIT, push 2026-09-18, 아카이브 False, 언어 TypeScript.
- 목록의 CC0은 이 저장소 코드에 미치지 않는다. 라이선스는 MIT이다.
- API 설명 한 줄: Claude Code plugin that replaces the compaction summary with Jev decisions: every tool call and result is scored in one fast request, stale ones are dropped or truncated, everythin

## Jev의 역할

Jev는 에이전트가 부르는 도구로 열려 있거나, 이미 문장을 쓰는 에이전트 앞의 게이트다. 목록은 이렇게 적는다. Claude Code 플러그인과 npm 라이브러리. 문맥을 요약하는 대신 Jev가 도구 호출에 점수를 매겨 낡은 것을 버린다. 에이전트 루프, 권한, 그 다음 일은 Jev가 아니다. 이 페이지는 그것들을 따라가지 않았다.
