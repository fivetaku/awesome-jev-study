...
- 목록: [kraayenjon/awesome-jev](https://github.com/kraayenjon/awesome-jev/blob/22570dcd8662ae039860a1dfad7d7aec4aff8e15/README.md#L244) `README.md:244` @ `22570dcd` (CC0). fivetaku/awesome-jev 포크는 이 커밋으로 클론했다.
- 절: 브라우저·컴퓨터 조작
- 주소: https://github.com/romaluev/jev-ego
- 조회: 2026-09-22
...
ego lite 위의 브라우저 에이전트. TypeSafe 요청 하나가 동작과 번호 매긴 요소를 고른다. 에이전트용 observe/act/suggest/step CLI가 있다.

## 확인한 것

- 실행으로 확인, GitHub API 2026-09-22: `romaluev/jev-ego`, 스타 15, SPDX NOASSERTION, push 2026-09-17, 아카이브 False, 언어 TypeScript.
- 목록의 CC0은 이 저장소 코드에 미치지 않는다. 라이선스는 NOASSERTION이다.
- API 설명 한 줄: Fast browser agent for ego lite. One TypeSafe request per step; an agent or Jev picks the move.

## Jev의 역할

목록이 맞다면 Jev의 자리는 브라우저나 컴퓨터 조작 루프 안의 결정이다. 목록은 이렇게 적는다. ego lite 위의 브라우저 에이전트. TypeSafe 요청 하나가 동작과 번호 매긴 요소를 고른다. 에이전트용 observe/act/suggest/step CLI가 있다. 페이지를 읽고 클릭을 실행하는 일은 일반 코드다. 이 페이지는 그 루프를 따라가지 않았고, 이 프로젝트가 픽셀을 본다고 말하지 않는다. 텍스트 상태이지 프레임이 아니라고 적은 것은 Doom 소개뿐이며, 그 문장을 다른 에이전트에 옮기지 않는다.
