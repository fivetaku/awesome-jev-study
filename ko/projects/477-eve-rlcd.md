...
- 목록: [kraayenjon/awesome-jev](https://github.com/kraayenjon/awesome-jev/blob/22570dcd8662ae039860a1dfad7d7aec4aff8e15/README.md#L477) `README.md:477` @ `22570dcd` (CC0). fivetaku/awesome-jev 포크는 이 커밋으로 클론했다.
- 절: 연구·오픈 모델
- 주소: https://github.com/anthony-maio/eve-rlcd
- 조회: 2026-09-22
...
Jev에서 영감을 받은 0.6B 결정 모델. 맞고 틀림 피드백만으로 강화학습한다. 보상은 결과에서 말한 확률을 뺀 값. 한 `state` 위에서 Choice·Score·Noul을 병렬로 답하고 글을 생성하지 않는다. RLVR 제거 실험과 [공개 가중치](https://huggingface.co/anthonym21/qwen3-0.6b-rlcd-decision)가 있다. TypeSafe 방법을 재현한 것이 아니라고 분명히 적는다.

## 확인한 것

- 실행으로 확인, GitHub API 2026-09-22: `anthony-maio/eve-rlcd`, 스타 6, SPDX MIT, push 2026-09-21, 아카이브 False, 언어 Python.
...
이 행은 주장 옆에 놓인 논문, 로컬 모델, 오픈 산출물이다. 목록은 이렇게 적는다. Jev에서 영감을 받은 0.6B 결정 모델. 맞고 틀림 피드백만으로 강화학습한다. 보상은 결과에서 말한 확률을 뺀 값. 한 `state` 위에서 Choice·Score·Noul을 병렬로 답하고 글을 생성하지 않는다. RLVR 제거 실험과 [공개 가중치](https://huggingface.co/anthonym21/qwen3-0.6b-rlcd-decision)가 있다. TypeSafe 방법을 재현한 것이 아니라고 분명히 적는다. Jev를 의존성으로 가정하지 않는다. 앞부분이 Jev의 이름을 말하는지까지 보았다. 소스는 따라가지 않았다.
