...
- 목록: [kraayenjon/awesome-jev](https://github.com/kraayenjon/awesome-jev/blob/22570dcd8662ae039860a1dfad7d7aec4aff8e15/README.md#L275) `README.md:275` @ `22570dcd` (CC0). fivetaku/awesome-jev 포크는 이 커밋으로 클론했다.
- 절: 개발 도구·코드 리뷰
- 주소: https://github.com/different-ai/openwork
- 조회: 2026-09-22
...
평가 테스트킷에 Jev를 검증 심판으로 넣어, 에이전트가 만든 결과물을 타입이 있는 판정으로 막는다. OpenWork 스타 수는 Jev 채택 수가 아니다. 생성일은 2026-01-14.

## 확인한 것

- 실행으로 확인, GitHub API 2026-09-22: `different-ai/openwork`, 스타 23689, SPDX NOASSERTION, push 2026-09-22, 아카이브 False, 언어 TypeScript.
- 목록의 CC0은 이 저장소 코드에 미치지 않는다. 라이선스는 NOASSERTION이다.
- API 설명 한 줄: The open-source alternative to Claude Cowork (powered by opencode)
- API 생성일 2026-01-14. 스타 수를 Jev 채택 규모로 읽지 않는다.
- 2026-09-22에 읽은 API 설명과 README 앞부분에서 Jev, TypeSafe, System One이라는 단어를 찾지 못했다. 연결은 목록 문장만 말한다.
- 목록은 eval testkit에 Jev를 검증 판정기로 넣었다고만 적는다 (README.md:275). 생성일 2026-01-14의 스타는 Cowork 대안의 크기지 Jev 채택 규모가 아니다.

## Jev의 역할

목록은 이렇게 적는다. 평가 테스트킷에 Jev를 검증 심판으로 넣어, 에이전트가 만든 결과물을 타입이 있는 판정으로 막는다. OpenWork 스타 수는 Jev 채택 수가 아니다. 생성일은 2026-01-14. 이 절에 모인 행은 대개 diff, 로그, 커밋, 이미 뽑아 둔 문자열에 대한 판단이고, 파일시스템과 git 호출은 일반 코드가 가진다. 그것은 절의 모양이다. 이 행이 그렇게 생겼다는 확인은 아니다. 이 페이지는 그 코드를 따라가지 않았다.
