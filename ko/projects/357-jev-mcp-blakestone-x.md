...
- 목록: [kraayenjon/awesome-jev](https://github.com/kraayenjon/awesome-jev/blob/22570dcd8662ae039860a1dfad7d7aec4aff8e15/README.md#L357) `README.md:357` @ `22570dcd` (CC0). fivetaku/awesome-jev 포크는 이 커밋으로 클론했다.
- 절: 에이전트 도구·MCP
- 주소: https://github.com/blakestone-x/jev-mcp
- 조회: 2026-09-22
...
Python MCP 서버. 분류, 점수, 확인, 맞추기, 거르기.

## 확인한 것

- 실행으로 확인, GitHub API 2026-09-22: `blakestone-x/jev-mcp`, 스타 19, SPDX MIT, push 2026-09-16, 아카이브 False, 언어 Python.
- 목록의 CC0은 이 저장소 코드에 미치지 않는다. 라이선스는 MIT이다.
- API 설명 한 줄: MCP server for TypeSafe Jev: typed classify, score, check, match and screen for any agent, with confidence on every answer

## Jev의 역할

Jev는 에이전트가 부르는 도구로 열려 있거나, 이미 문장을 쓰는 에이전트 앞의 게이트다. 목록은 이렇게 적는다. Python MCP 서버. 분류, 점수, 확인, 맞추기, 거르기. 에이전트 루프, 권한, 그 다음 일은 Jev가 아니다. 이 페이지는 그것들을 따라가지 않았다.
