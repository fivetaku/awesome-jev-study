...
- 목록: [kraayenjon/awesome-jev](https://github.com/kraayenjon/awesome-jev/blob/22570dcd8662ae039860a1dfad7d7aec4aff8e15/README.md#L365) `README.md:365` @ `22570dcd` (CC0). fivetaku/awesome-jev 포크는 이 커밋으로 클론했다.
- 절: 에이전트 도구·MCP
- 주소: https://github.com/DevMortimer/pi-warden
- 조회: 2026-09-22
...
pi-typesafe 위의 Pi 가드. 대화 대신 붙잡아 둔 도구 결과. 쓰기는 프로젝트 규칙 파일에 비춘다.

## 확인한 것

- 실행으로 확인, GitHub API 2026-09-22: `DevMortimer/pi-warden`, 스타 132, SPDX MIT, push 2026-09-22, 아카이브 False, 언어 TypeScript.
- 목록의 CC0은 이 저장소 코드에 미치지 않는다. 라이선스는 MIT이다.
- API 설명 한 줄: Guardrails for Pi that steer instead of interrupt: enforces your project rules on every write, holds only hard-to-undo actions (3 per 1,000 calls, measured on real sessions), catch

## Jev의 역할

Jev는 에이전트가 부르는 도구로 열려 있거나, 이미 문장을 쓰는 에이전트 앞의 게이트다. 목록은 이렇게 적는다. pi-typesafe 위의 Pi 가드. 대화 대신 붙잡아 둔 도구 결과. 쓰기는 프로젝트 규칙 파일에 비춘다. 에이전트 루프, 권한, 그 다음 일은 Jev가 아니다. 이 페이지는 그것들을 따라가지 않았다.
