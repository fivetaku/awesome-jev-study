...
각 페이지는 목록 문장, 확인된 사실, Jev의 역할, 세팅, LLM이었다면, 스크립트였다면.
...
- [Submit yours →](projects/018-submit-yours.md) — README.md:18 — 제출 페이지는 링크 하나를 받는다. 페이지는 Jev가 그것을 읽고, 실릴 자격이 있는지 판단하고, 분류한다고 한다. 등재는 무료이고, 돈을 내고 자리를 살 수 없다고 페이지가 말한다.
- [madewithjev.com](projects/018-madewithjev-com.md) — README.md:18 — madewithjev.com은 사람들이 Jev로 만드는 것의 디렉터리다. 목록은 빌드마다 저자가 보고한 비용, 지연, 출처가 붙는다고 한다.
...
- [Jev vs LLM 표](projects/074-jev-vs-llm.md) — README.md:74 — 목록의 표(README.md:74-85). 목록은 이것을 런치 글에서 가져왔다고 한다. 기존 LLM 대 System One + Jev. RLHF·RLVR 대 RLCD, 파싱이 필요한 문자열 대 타입이 있는 값, 토큰 순차 대 한 번의 병렬 질의, 입력 $0.20–$10 / MTok...

## Pricing, limits, and access — 가격·한도·접근

- [가격·한도 표](projects/091-item.md) — README.md:91 — 목록의 가격 표(README.md:91-100). 2026-09-18 스냅샷이라고 스스로 적는다. `jev-latest`는 `jev-1.13.0`. 엔드포인트 `POST https://api.typesafe.ai/v1/systemone`. 입력 100만 토큰당 $0.042, 출력 ...
...
- [퀵스타트 예제](projects/102-item.md) — README.md:102 — 목록의 퀵스타트(README.md:102-159). 여기서 실행하지 않았다. Python `pip install typesafe-sdk`, 클래스 `Choice`, `Noul`, `Score`, `TypeSafeClient.system_one`, 키는 `TYPESAFE_API_KE...
...
## Featured builds with real numbers — 숫자가 붙은 빌드
...
## Browser and computer-use agents — 브라우저·컴퓨터 조작
...
## Search, retrieval, and data — 검색·데이터
...
## Developer tools and code review — 개발 도구·코드 리뷰
...
## Business and vertical apps — 업무·버티컬 앱
...
## Agent tools and MCP servers — 에이전트 도구·MCP
...
## Use cases by industry — 산업별 유스케이스
...
## Benchmarks and evaluations — 벤치마크
...
## Research and open models — 연구·오픈 모델
